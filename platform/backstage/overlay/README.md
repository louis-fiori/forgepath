...
| Path | Purpose |
|---|---|
| `gitops/platform/backstage/` | k8s manifests for Backstage itself (Namespace, Deployment, Service, RBAC). Reconciled by ArgoCD via the `backstage` Application generated by the `platform` ApplicationSet (`platform/argocd/bootstrap/platform-appset.yaml`). |
