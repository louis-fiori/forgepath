...
case "${RUNTIME}" in
  ...
  docker-desktop)
    ...
    kubectl --context docker-desktop delete namespace \
      ...
      --ignore-not-found
    # Cluster-scoped RBAC isn't removed by deleting namespaces. incident-analyzer-reader
    # (used by the analyzer's cross-namespace pod/event reads) was being orphaned.
