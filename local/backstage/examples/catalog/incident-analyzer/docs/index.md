...
| `BEDROCK_MODEL_ID` / `AWS_REGION` | `anthropic.claude-sonnet-4-6` / `us-east-1` | Bedrock model + region |
...
**From the CLI:**
```bash
kubectl -n incident-analyzer port-forward svc/incident-analyzer 8889:80 &
...
curl "${AUTH[@]}" "localhost:8889/analyze?namespace=incident-generator&force=true"
# Analyze WITHOUT opening an issue (per-request override):
curl "${AUTH[@]}" "localhost:8889/analyze?namespace=incident-generator&force=true&open_issue=false"
curl localhost:8889/metrics   # /metrics, /healthz, /readyz and / are not gated
```
...
- **Globally at runtime** (instant, no redeploy):
  ```bash
  curl "${AUTH[@]}" -XPOST localhost:8889/settings -d '{"issues_enabled": false}'  # turn off
  curl "${AUTH[@]}" -XPOST localhost:8889/settings -d '{"issues_enabled": true}'   # back on
  curl "${AUTH[@]}" localhost:8889/settings                                        # check
  ```
